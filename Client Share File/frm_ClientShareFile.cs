...
namespace Client_Share_File
{
    public partial class frm_ClientShareFile : Form
    {
        ...
        private void btn_GetFile_Click(object sender, EventArgs e)
        {
            ...
            client.Connect(ipe);
            byte[] bFilePath = Encoding.Unicode.GetBytes(txt_GetFile.Text);
            client.GetStream().Write(bFilePath, 0, bFilePath.Length);
            FileStream fstream = new FileStream(Application.StartupPath + txt_GetFile.Text.Substring(txt_GetFile.Text.IndexOf("\\")), FileMode.Create);
            byte[] bFile = new byte[client.ReceiveBufferSize];
            new Thread(() =>
            {
                while (true)
                {
                    int count = client.GetStream().Read(bFile, 0, bFile.Length);
